//! POST live ids to `/streaming` and VOD catalog to `/vods`.
...
#[derive(Debug, Serialize)]
struct VodsPostBody<'a> {
    items: &'a [CatalogItem],
}

fn basic_auth(post_auth: &str) -> String {
    let credentials = STANDARD.encode(format!("admin:{post_auth}"));
    format!("Basic {credentials}")
}

/// `.../streaming` → `.../vods`
pub fn vods_url(post_url: &str) -> String {
    match post_url.rfind('/') {
        Some(i) => format!("{}vods", &post_url[..=i]),
        None => "vods".into(),
    }
}
...
pub async fn post_to_worker(
    ...
    let resp = client
        .post(post_url)
        .header("Authorization", basic_auth(post_auth))
...
pub async fn post_vods(
    client: &reqwest::Client,
    post_url: &str,
    post_auth: &str,
    items: &[CatalogItem],
) -> Result<serde_json::Value> {
    let url = vods_url(post_url);
    tracing::info!("Worker POST /vods items={} url={url}", items.len());
    ...
    let resp = client
        ...
        .header("Cache-Control", "no-store")
        .header("User-Agent", WORKER_USER_AGENT)
        .json(&VodsPostBody { items })
        .timeout(std::time::Duration::from_secs(30))
        .send()
        .await
        .context("POST worker /vods")?;
    ...
    if !status.is_success() {
        anyhow::bail!("Worker /vods HTTP {status}: {payload}");
    }

    tracing::info!("Worker /vods HTTP {status} items={}", items.len());
    Ok(payload)
}
